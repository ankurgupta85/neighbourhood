...
CREATE TABLE IF NOT EXISTS `messages` (
  ...
  CONSTRAINT `FK_messages_parent_messages` FOREIGN KEY (`prev_message_id`) REFERENCES `messages` (`message_id`),
...
# Dumping structure for table neighbours.search_name
DROP TABLE IF EXISTS `search_name`;
CREATE TABLE IF NOT EXISTS `search_name` (
  `username` varchar(100) NOT NULL,
  `First_Name` varchar(100) NOT NULL,
  `Last_Name` varchar(100) NOT NULL,
  KEY `FK__search_users` (`username`),
  CONSTRAINT `FK__search_users` FOREIGN KEY (`username`) REFERENCES `search_users` (`Username`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
