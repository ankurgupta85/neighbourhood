...
<%@page import="java.util.List,com.neighbourhood.event.*"%>
...
<body>
	<center>
		...
		<%
			...
			if (session == null || session.getAttribute("username") == null) {
				...
				List<EventInfo> allFutureEvents = ShowEvents
						.getFutureEvents(username);

				if (allFutureEvents.isEmpty()) {
		%>
		<font size="2" color="black"
			style="font-family: cursive; background-color: white;">No
		...
		<%
			// Implement paging...
					...
					int numRows = allFutureEvents.size();
					...
					for (int inum = 1; inum <= numPages; inum++) {
						temp = temp
								+ "<a href=\"showUpcomingEvents.jsp?startIndex="
 ...
 --%>
		<table border="0" width="80%">
			<tr>
				<td width="43%" scope="col" class="HdrParagraph" align="left">
					<%
						if (startIndex != 0) {
					%> <a
					href="showUpcomingEvents.jsp?startIndex=<%=startIndex - numRecordsPerPage%>&txtnumrec=<%=numRecordsPerPage%>"
		...
		<table border="0" align="center" cellpadding="3" cellspacing="3" width="80%">
			...
			<%
				for (int index = startIndex; index < increment; index++) {
								...
								EventInfo event = allFutureEvents.get(index);
			...
			<tr>
				<td width="auto"><a
					href="eventDetails.jsp?event_id=<%=event.getEvent_id()%>;FUTURE"
				...
				<td width="auto"><font size="2" color="black"
					style="font-family: cursive; background-color: white;"><%=event.getDate()%>
						<%=event.getTime()%></font></td>
				<td width="auto"><a href="otheruser.jsp?username=<%=event.getCreator()%>"
					style="text-decoration: none;"><font size="2" color="black"
