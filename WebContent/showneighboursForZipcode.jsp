...
<body bgcolor="white">
	<center>
		...
		<%
			String zipcode = request.getQueryString().split("=")[1];
		...
		<br> <br> <b><font size="2"
			style="font-family: cursive; color: black; background-color: white;">Neighbours
				currently located in <%=zipcode%></font></b>
		...
		<%
			...
			if (session == null || session.getAttribute("username") == null) {
				...
				List<User> neighbourList = Neighbours
						.getNeighboursList(username);
		...
		<%
					...
					List<User> neighbourListByZipcode = (ArrayList<User>) GetParsedNeighbours
							.getParsedNeighboursByZipcode(neighbourList,
									zipcode);

					if (neighbourListByZipcode.isEmpty()) {
		%>
		<font size="2"
			style="font-family: cursive; color: black; background-color: white;">You
			dont have any neighbours currently located in zipcode <%=zipcode%>.
			Please check for your neighbours in other zipcodes.
		</font>
		<%
			} else {
		%>
		...
		<%
			// Implement paging...
						...
						int numRows = neighbourListByZipcode.size();
						...
						for (int inum = 1; inum <= numPages; inum++) {
							temp = temp
									+ "<a href=\"showneighbours.jsp?startIndex="
									+ startpoint + "&txtnumrec="
									+ numRecordsPerPage + "\" >" + inum
									+ "</a> ";
 ...
 %>
		...
		<table border="0" align="center" cellpadding="3" cellspacing="3"
			...
			<%
				// if neighbour List is not empty.
							for (int index = startIndex; index < increment; index++) {
								User user = neighbourListByZipcode.get(index);
			...
			<td width="auto"><font size="2"
				style="font-family: cursive; color: black; background-color: white;"><a
					href="<%=unblockupdates%>"
					style="color: black; font-size: 2; text-decoration: none;"
