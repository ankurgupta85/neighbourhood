	...
	<center>
		...
		/* 	if (session.getAttribute("eventTopic") != null) {
			...
			if (session == null || session.getAttribute("username") == null) {
				...
				List<EventInfo> allTodayEvents = ShowEvents
						.getTodayEvents(username);

				if (allTodayEvents.isEmpty()) {
		%>
		<font size="2" color="black"
			style="font-family: cursive; background-color: white;">No
			Events scheduled for today</font>
		<%
			} else {
		%>
		...
		<%
			// Implement paging...
					...
					int numRows = allTodayEvents.size();
					...
					for (int inum = 1; inum <= numPages; inum++) {
						temp = temp
								+ "<a href=\"showTodayEvents.jsp?startIndex="
			...
			<tr>
				<td width="43%" scope="col" class="HdrParagraph">
					<%
						if (startIndex != 0) {
					%> <a
					href="showTodayEvents.jsp?startIndex=<%=startIndex - numRecordsPerPage%>&txtnumrec=<%=numRecordsPerPage%>"
		...
		<table border="0" align="center" cellpadding="3" cellspacing="3" width="80%">
			...
			<%
				for (int index = startIndex; index < increment; index++) {
								...
								EventInfo event = allTodayEvents.get(index);
			...
			<tr>
				<td><a
					href="eventDetails.jsp?event_id=<%=event.getEvent_id()%>;TODAY"
